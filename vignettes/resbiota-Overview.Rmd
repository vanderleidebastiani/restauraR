...
resCheck <- checkReference(reference = referenceSites, # Species composition in reference sites
                           ...
                           supplementary = supplementarySites, # Species composition in supplementary sites
...
scenarioA <- simulateCommunities(traits = traits, # Species traits data
                                 ava = "Available", # Column name that indicates the availability of species
                                 it = 100, # Number of simulations
                                 rich = c(10, 15),  # Species richness constraints, min = 10, max = 15
                                 maxDiver = c("BT", "SLA", "Height", "Seed"), # Maximise functional diversity of all traits
                                 constCWM = "BT", # Constrain communities by bark thickness trait
                                 method = "proportions", # Sampling method
                                 phi = 1, # Weight of quadratic entropy or entropy, phi = 1 to maximise functional diversity
...
scenarioB <- simulateCommunities(traits = traits, 
                                 restComp = restorationSites, # Species composition for restoration sites
                                 ava = "Available", 
                                 it = 100, 
                                 rich = c(10, 15), 
...
allScenarios <- computeParameters(x = allScenarios,
                                  ...
                                  cost = "Cost", # Column name that indicates the cost per individual
                                  dens = "Density", # Column name that species planting density
                                  dissimilarity = c("SLA", "Height", "Seed"), # Traits for dissimilarity calculation
                                  reference = referenceSites, # Species composition in the reference site
                                  supplementary = supplementarySites) # Species composition in the supplementary sites
allScenarios
```
...
allScenariosAlternative <- computeParameters(x = allScenarios,
                                             ...
                                             ava = "Available",
                                             cwm = "BT",
                                             rao = list2rao, # List with the two sets of trait names
                                             cost = "Cost",
                                             dens = "Density",
...
allScenarios <- standardiseParameters(allScenarios,
                                      parameters = c("dissimilarity", 
                                                     "functionalDissimilarity"), # Parameters to standardise
                                      method = "max")
allScenarios
```
...
allScenarios <- computeMultifunctionality(x = allScenarios, 
                                          tests = c("CWM_BT > 6",
                                                    "rao > 0.25")) # Logical tests for multifunctionality assessment
...
priorityTargets <- c("CWM_BT > 6", "rao > 0.25", "Cost == 'MIN'") # Logical tests for priority selection
resSelectSimPriority <- selectCommunities(allScenarios, 
                                          testsPriority = priorityTargets,
                                          siteGroup = "Scenario", # Column name to define site groups
                                          singleSelection = TRUE) # Select only one simulation by site group
...
scenarioMultisite <- optimiseSelection(resSelectSimPart2,
                                       ...
                                       includeReference = TRUE, # Include reference sites in index calculations
                                       traits = traits,
                                       rao = c("SLA", "Height", "Seed"), # Traits for Rao calculation
                                       calcSimpsonBeta = TRUE) # Calculate Simpson's beta diversity
scenarioMultisite
...
```{r}
scenarioMultisiteSelected <- selectCommunities(scenarioMultisite,
                                               testsMultisite = "alphaFunctionalDiversity == 'MAX'") # Logical tests for multi-site selection
scenarioMultisiteSelected
```
...
extractResults(scenarioMultisiteSelected, 
               type = "simResults")
```
