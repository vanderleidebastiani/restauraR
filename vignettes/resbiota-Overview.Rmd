...
The resbiota package contains an integrative set of tools to create restoration solutions by assemble communities from a regional species pool. The main functions are demonstrated below using an illustrative example. Set the seed to reproduce the same simulation.
...
The `cerrado.mini` is a toy dataset containing only 50 sampled species of the full `cerrado` dataset. The original data set includes traits of 483 species used in restoration projects in the savannas in the Cerrado biome. This toy dataset is designed to illustrate the main features of the resbiota package, including scenarios with reference sites, restoration sites and supplementary sites.

The dataset is arranged as a list: the `traits` object contains traits data, the `reference` object contains the composition of 2 reference sites, the `restoration` object contains the composition of 2 restoration sites and the `supplementary` contains the composition of 2 supplementary sites. The supplementary sites are similar to reference sites but included for illustrative purposes, allowing parameters to be calculated and plotted to aid interpretation.
...
allScenarios <- computeParameters(x = allScenarios,
                                  ...
                                  # Species composition in the supplementary sites
                                  supplementary = supplementarySites) 
```
