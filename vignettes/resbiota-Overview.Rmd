...
The function computeMultifunctionality is optional and computes the matrix of multifunctionality and alpha multifunctionality index. See the multifunctionality topic below.
...
The dataset is arranged as a list: the `traits` object contains traits data, the `reference` object contains the composition of 2 reference sites, the `restoration` contains composition of 2 restoration sites and the `supplementary` contains composition of 2 supplementary sites. The supplementary sites are similar to reference sites but included for illustrative propose, allowing parameters to be calculated and plotted to aid interpretation.
...
The `checkReference` function checks the species pool and functional parameters in reference sites calculating basic functional parameters and returning summary statistic. The function return an object of class `simRestCheck` (a list object) with the descriptive statistics.
...
The simulation are performed using the `simulateCommunities` function by specifying input parameters. The function return an object of class `simRest` (a list object) with basic information on input parameters and a list of results.
...
The multifunctionality also can be computed after the basic parameter calculation. Note that the alpha multifunctionality index are included in the result data frame.
...
The `viewResults` function return a `ggplot` object, so additional function from the `ggplot2` package can be used to customize the plots, such as titles, labels and plot themes.
