...
print.simRest <- function(x, ...) {
  ...
  cat(" Multisite parameters: ")
  if(!is.null(x$simulation$multisite$results)) {
    cat("\n")
    cat(paste("   ", colnames(x$simulation$multisite$results)), sep = "\n")
    cat("\n")
  } else{
    cat(ifelse(is.null(x$simulation$multisite$results), "No", "Yes"), "\n\n")
  }
  ...
  if(!is.null(x$simulation$multisite$results)) x$simulation$multisite$results <- as.matrix(x$simulation$multisite$results)
