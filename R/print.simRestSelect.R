...
print.simRestSelect <- function(x, ...) {
  ...
  if(!is.null(x$selection$multisite$results)) {
    cat("\n")
    cat(paste("   ", colnames(x$selection$multisite$results)), sep = "\n")
    cat("\n")
  } else{
    cat(ifelse(is.null(x$selection$multisite$results), "No", "Yes"), "\n\n")
  }
  ...
  if(!is.null(x$selection$multisite$results)) x$selection$multisite$results <- as.matrix(x$selection$multisite$results)
